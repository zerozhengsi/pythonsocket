...
def connect_pro(params, param, epoll_fd, datas, fd, cur_time, host, port):
    try:
        ...
        if not datas.startswith("CONNECT"):
            ...
            epoll_fd.register(next_fileno, epoll.EPOLLIN | epoll.EPOLLOUT | epoll.EPOLLERR | epoll.EPOLLHUP)
            epoll_fd.modify(fd, epoll.EPOLLIN | epoll.EPOLLERR | epoll.EPOLLHUP)
        else:
            next_param = {"addr":[host,port],"writelen":0, "connections":nextfd, "time":cur_time, "nextfd":fd}
            param["nextfd"] = next_fileno
            next_param["next"] = "true"
            params[next_fileno] = next_param
            epoll_fd.register(next_fileno, epoll.EPOLLIN | epoll.EPOLLERR | epoll.EPOLLHUP)
            param["read_len"] = 0
            param["readdata"] = ""
            param["writedata"] = "HTTP/1.1 200 Connection Established\r\nConnection: close\r\n\r\n"
            param["writelen"] = 0
            param["reuse"] = "true"
            epoll_fd.modify(fd, epoll.EPOLLIN | epoll.EPOLLOUT | epoll.EPOLLERR | epoll.EPOLLHUP)
...
def process_datas(process_status, params, param, epoll_fd, datas, read_len, fd, cur_time):
    if process_status == "close":
        ...
        if need_connect or not "nextfd" in param:
            ...
            if host_s > 0 and host_e > 0:
                if not datas.startswith("CONNECT"):
                    epoll_fd.modify(fd, epoll.EPOLLERR | epoll.EPOLLHUP) # 简单处理，http连接时把读去掉，避免内存攻击
            ...
            param["read_len"] = 0
            param["readdata"] = ""
            epoll_fd.modify(next_fileno, epoll.EPOLLIN | epoll.EPOLLOUT | epoll.EPOLLERR | epoll.EPOLLHUP)
...
def run_main(listen_fd):
    ...
    while True:
        ...
        for fd, events in epoll_list:
            if fd == listen_fd.fileno():
                while True:
                    try:
                        ...
                        conn.setblocking(0)
                        epoll_fd.register(conn.fileno(), epoll.EPOLLIN | epoll.EPOLLERR | epoll.EPOLLHUP)
                        conn.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                        #conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, True)
                        params[conn.fileno()] = {"addr":addr,"writelen":0, "connections":conn, "time":cur_time}
                    except socket.error, msg:
                        break
            elif epoll.EPOLLIN & events:
            ...
            elif epoll.EPOLLHUP & events or epoll.EPOLLERR & events:
                clearfd(epoll_fd,params,fd)
                logger.error("sock: %s error" % fd)
            elif epoll.EPOLLOUT & events:
                param = params.get(fd,None)
                if param == None:
                    continue
                param["time"] = cur_time
